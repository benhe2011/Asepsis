...
async function getData(name, lat, lng) {
  var url =
    'https://api.breezometer.com/baqi/?lat=' +
    lat +
    '&lon=' +
    lng +
    '&key=' +
...
function setMarkers(map) {
  ...
  for (var i = 0; i < campuses.length; i++) {
    ...
    google.maps.event.addListener(newMarker, 'click', function() {
      ...
      map.setZoom(8);
      document.getElementById('insertAQI').innerHTML =
        myData[this.getTitle()].country_aqi;
      document.getElementById('insertCOND').innerHTML =
        myData[this.getTitle()].country_description;
      document.getElementById('insertPOL').innerHTML =
        myData[this.getTitle()].dominant_pollutant_description;
      displayOnClick();
